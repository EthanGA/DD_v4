...
public class Unit : MonoBehaviour {
    ...
    void Update() {
        ...
        if (transform.position.x >= target.position.x - 2f && transform.position.x <= target.position.x + 2f) {
            if (transform.position.z >= target.position.z - 2f && transform.position.z <= target.position.z + 2f) {
                giveUp = true;
